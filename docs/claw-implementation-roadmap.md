...
### Phase 8D: ロール型エージェントモード（計画中）
- 軽量ロール/モードシステム
- モード固有のプロンプト拡張
- セッション単位のモード永続化

### Phase 8E: メモリ優先順位付け（計画中）
- コンテキスト増加時のメモリランキング
- 関連性に基づく選択的メモリ含入
- 長時間セッション向け安全切詰

### Phase 8F: ウェブ検索（最小）（計画中）
- 特定クエリ用オプション軽量ウェブ検索
- メモリ・文脈統合
- 明示的ユーザーコントロール

### Phase 8G: Git 読取ツール（計画中）
...
- ✅ API層（`claw-ui/local-api`）：安定、`attachments`・メモリ注入用に拡張
...
### 作業位置
Phase 8B（プロンプト注入）完了。Phase 8C（モデル選択）へ次に進める準備完了。

## 次の実装優先順位

### 1. Phase 8C: モデル選択（次）
- セッション単位のモデル選択UI
- `local-api` truth 所有権を破らずモデル配線
- アクティブモデル設定との互換性維持

### 2. Phase 8D: ロール型エージェントモード（次々）
- 軽量ロール/モード（例："code-focused", "research-mode"）
- モード固有のプロンプト拡張
- セッション単位のモード永続化

### 3. Phase 8E: メモリ優先順位付け（次々）
- コンテキスト増加時のメモリランキング
- 関連性に基づく選択的メモリ含入
- 長時間セッション向け安全切詰

### 4. Phase 8F: ウェブ検索（最小）（次々）
- 特定クエリ用オプション軽量ウェブ検索
- メモリ・文脈統合
- 明示的ユーザーコントロール
...
| `local-api` | 安定 | 実行/ログ状態所有、`attachments` + `projectMemory` フィールド付き拡張 RunRequest 受け入れ |
...
| 実行連携 | 完了 | メモリ + 添付を実行リクエスト経由で配線、アダプターが最小プロンプト拡張で注入 |
| 次フェーズ入口 | Phase 8C | セッション単位のモデル選択、`local-api` truth 所有権を維持 |
...
2. 次の作業は **Phase 8C: モデル選択**
   - `claw-studio` にセッション単位のモデル選択 UI を設計
   - `local-api` を経由した選択配線（状態所有権は保持）
   - アクティブモデル設定との下位互換性を確保
3. Phase 8C 境界：`claw-studio` はモデル選択肢を準備して送信、`local-api` が検証・配線
4. すべての Spec 変更は JP/EN 両版を更新すること
5. 実装は最小限・可逆的に保つ

**現在のテストエントリーポイント:**
- `apps/claw-studio` で `npm run typecheck && npm run build` を実行
- 添付ファイル処理（ペースト、ドラッグ、ピッカー）の回帰なしを確認
- 手動テスト：画像をペースト、ドラッグ、ピッカーで選択；すべてチップで表示されることを確認
- アシスタント応答コピーが機能することを確認
- 実行時のメモリ注入を確認（ログで "[v1 injection]" メッセージを確認）

---

**最終更新**: 2026-04-11  
**整合対象**: `claw-handover-spec.md` (latest)  
**実装状態**: Phase 8B 完了、Phase 8C 準備完了
