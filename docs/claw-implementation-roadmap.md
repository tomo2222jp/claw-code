...
### 作業位置
Phase 8E（メモリ優先順位付け）完了。Phase 8F（ウェブ検索 最小）へ次に進める準備完了。

## 次の実装優先順位

### 1. Phase 8F: ウェブ検索（最小）（次）
- 特定クエリ用オプション軽量ウェブ検索
- メモリ・文脈統合
- 明示的ユーザーコントロール

### 2. Phase 8G: Git 読取ツール（次々）
- 読取専用 git log・diff・blame アクセス
- 現在プロジェクトにスコープ限定
- 安全なサブプロセス処理
...
| 次フェーズ入口 | Phase 8F | ウェブ検索（最小）、明示的ユーザーコントロール・スコープ結果 |
...
2. 次の作業は **Phase 8F: ウェブ検索（最小）**
   - 特定ユーザークエリ向けの軽量検索機能を実装
   - 明示的なユーザーコントロールとスコープ限定結果のみ
   - メモリ・文脈との統合
3. Phase 8F 境界：`claw-studio` が検索開始、`local-api` またはアダプターがクエリ配線
...
- 実行パイプラインの回帰なしを確認
- 手動テスト：モデル選択・ロール型モード・メモリ優先順位付けをログで確認
- デバッグログで "[v1 injection]"（メモリ）、"[v1 role]"（ロール）、"[v1 memory]"（優先順位付け）メッセージを確認
...
**実装状態**: Phase 8E（メモリ優先順位付け）完了、Phase 8F（ウェブ検索 最小）準備完了
