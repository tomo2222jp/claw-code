...
export class ClawEngineAdapter implements EngineAdapter {
  ...
  startRun(run: EngineAdapterRun, callbacks: EngineAdapterCallbacks): RunningEngineHandle {
    ...
    // Use resolved model from llmSettings if available, otherwise fallback to activeModel
    const resolvedModel = run.settings.llmSettings?.modelId || run.settings.activeModel;
    const args: string[] = [
    ...
    // Use resolved values from llmSettings if available
    const resolvedProvider = run.settings.llmSettings?.provider || run.settings.activeProvider;
    const loggingResolvedModel = run.settings.llmSettings?.modelId || run.settings.activeModel;
    callbacks.onLog(
      "system",
      `claw settings: provider=${resolvedProvider} model=${loggingResolvedModel} retryCount=${run.settings.retryCount}`,
    );
...
function buildChildEnv(settings: EngineAdapterRun["settings"]): NodeJS.ProcessEnv {
  ...
  // Use resolved llmSettings values if available
  const llmSettings = settings.llmSettings;
  if (llmSettings) {
    const provider = llmSettings.provider || settings.activeProvider;
    const baseUrl = llmSettings.baseUrl;
    const apiKeys = llmSettings.apiKeys;

    // Set provider-specific environment variables
    if (baseUrl && baseUrl.trim() !== "") {
      // Set the appropriate BASE_URL environment variable based on provider
      if (provider === "anthropic") {
        env.ANTHROPIC_BASE_URL = baseUrl;
      } else if (provider === "openai" || provider === "openrouter" || provider === "custom") {
        // OpenAI-compatible providers (including custom) use OPENAI_BASE_URL
        env.OPENAI_BASE_URL = baseUrl;
      } else if (provider === "google") {
        // Google might use a different base URL environment variable
        // For now, set OPENAI_BASE_URL as fallback
        env.OPENAI_BASE_URL = baseUrl;
      }
    }
    ...
    } else if (llmSettings.apiKey && typeof llmSettings.apiKey === "string" && llmSettings.apiKey.trim() !== "") {
      // Fallback to top-level apiKey field for backward compatibility
      if (provider === "anthropic") {
        env.ANTHROPIC_API_KEY = llmSettings.apiKey;
      } else if (provider === "openai" || provider === "openrouter" || provider === "custom") {
        env.OPENAI_API_KEY = llmSettings.apiKey;
      } else if (provider === "google") {
        env.GOOGLE_API_KEY = llmSettings.apiKey;
      }
    }
  } else {
